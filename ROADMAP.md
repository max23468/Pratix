...
| ✅    | **Strategia test automatizzati progressiva**  | Baseline Vitest, `npm test`, `npm run test:coverage`, `npm run test:coverage:global`, piano in `docs/plans/strategia-test-automatizzati.md`, target ideale 75% linee operative e 45% linee globali secondarie, 39 unit test su calcoli/XML fattura, Storage, Prezzi, rendiconti Excel, changelog, label, schemi auth e helper server, integrazione in `ci:local`, Quality e pre-push guard |
| 🟡    | Test automatici recupero crediti              | Coperti template Prezzi, rendiconti Excel, calcoli/XML, label Attività, schemi auth e helper di fatturazione server; restano numero pratica, snapshot prezzi, rinvii, blocco attività fatturate, import, componenti/hook UI e RLS                                                                                                                                                          |
...
1. **Test recupero crediti server-side**: estendere la baseline Vitest a numero pratica, snapshot prezzi, rinvii, blocco attività fatturate, import e RLS.
