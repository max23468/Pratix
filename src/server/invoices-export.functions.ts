...
import type { BillingExportKind } from "@/lib/billing-xlsx";
import { buildInvoiceXml } from "@/lib/invoice-xml";
import { PRATIX_DOCUMENTS_BUCKET } from "@/lib/storage-paths";
import { billedPartyForInvoiceXml, type InvoiceXmlPrincipal } from "@/server/invoice-billing.logic";
...
export const generateBillingExportFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { exportReference, file } = await buildStoredBillingExport({
      supabase,
      userId,
