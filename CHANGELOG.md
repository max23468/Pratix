...
### Sotto il cofano
- **CI GitHub Actions minimale**: aggiunto workflow `Quality` su pull request verso `main` e avvio manuale. Esegue `npm ci` e `npm run build` su Linux, con skip per modifiche solo documentali.
...
- **Templates issue/PR e Dependabot**: aggiunti `.github/ISSUE_TEMPLATE/` (bug, idea), `PULL_REQUEST_TEMPLATE.md`, `dependabot.yml` (npm settimanale, minor/patch raggruppati; GitHub Actions mensile).
