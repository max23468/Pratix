...
export const isRetryableGitHubResponse = (status, remaining, retryAfter, message = "") =>
  status === 429 ||
  status >= 500 ||
  (status === 403 &&
    (remaining === "0" ||
...
async function reviewSignals(repository, number, requestedAt, invocationId) {
  ...
  const invocationReactions = invocation
    ...
    : [];
  const latestEyesAt = [...reactions, ...invocationReactions]
    .filter(
      (reaction) =>
        reaction.user?.login === CODEX_BOT &&
        reaction.content === "eyes" &&
  ...
  return [
    ...
    reviewComments,
    invocationReactions,
    invocation?.created_at ?? (latestEyesAt || requestedAt),
  ];
}
...
async function main() {
  ...
  for (let attempt = 0; attempt < CODEX_REVIEW_POLLING.attempts; attempt += 1) {
    ...
    const [comments, reactions, reviews, reviewComments, exactReactions, attemptRequestedAt] =
      signals;
