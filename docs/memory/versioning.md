...
La pubblicazione completa non impone sempre il gate tecnico massimo. Scegliere prima la corsia: veloce per docs interne, roadmap, ADR, regole agenti, memoria o `Non versionato` non esposto in app; standard per changelog, testi pubblici, microcopy esposta, piccola UI o contenuti mostrati in pagina; completa per parser, automazione release, routing, componenti condivisi, auth, database, dipendenze o UI sostanziale. La corsia decide build/lint/test/smoke, ma non elimina PR/merge, verifica Vercel proporzionata e cleanup quando il proprietario chiede "pubblica". `publish:finish` chiude il post-merge aggiornando main, verificando produzione via `VERCEL_TOKEN` o Portachiavi macOS `pratix.vercel.token`, controllando route non vuote e pulendo branch/worktree dedicati con operazioni sicure.
