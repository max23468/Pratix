...
La formattazione usa **oxfmt** (config in `.oxfmtrc.json`), non più Prettier: `npm run format:changed:check` verifica solo i file tracciati cambiati e `npm run format:changed` corregge solo quei file. Lo script `format-changed.mjs` filtra alle estensioni che oxfmt sa formattare (oxfmt non ha un equivalente di `prettier --ignore-unknown`). La formattazione non passa più da ESLint: `eslint-plugin-prettier` è stato rimosso, così lint e format restano separati. Il workflow GitHub usa lo stesso controllo sui file modificati prima di build/lint.
