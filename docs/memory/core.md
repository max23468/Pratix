...
Aggiornare [`docs/ROADMAP.md`](../ROADMAP.md) solo quando una decisione cambia
direzione, priorità, fase o backlog; usare ADR, guide o memoria per decisioni
durevoli e dettagli storici.
...
Il pre-push usa `npm run prepush:guard`: seleziona i controlli in base al diff tracciato, ignora file non tracciati, usa `origin/main` quando una branch non ha upstream, calcola la fingerprint dal contenuto dei diff e salva una cache locale, così un push ripetuto non rilancia format/build/lint/audit senza motivo. Qualunque modifica a `package.json` o `package-lock.json` fa partire `npm audit --audit-level=moderate`, incluse le sole rigenerazioni del lockfile. I check indipendenti girano in fasi parallele; `PRATIX_CHECKS_SEQUENTIAL=1` forza il modo sequenziale se serve diagnosticare log o flakiness. `PRATIX_SKIP_PREPUSH=1 git push` è ammesso solo quando i controlli equivalenti sono già stati eseguiti sullo stesso diff e il motivo viene dichiarato.

La toolchain lint+format è **oxc**, non più ESLint/Prettier. La formattazione usa **oxfmt** (config in `.oxfmtrc.json`): `npm run format:changed:check` verifica solo i file tracciati cambiati e `npm run format:changed` corregge solo quei file. Lo script `format-changed.mjs` filtra alle estensioni che oxfmt sa formattare (oxfmt non ha un equivalente di `prettier --ignore-unknown`). Il lint usa **oxlint** (config in `.oxlintrc.json`, `npm run lint` = `oxlint`): plugin `typescript` + `react` (che porta react-hooks nativo: `rules-of-hooks` error, `exhaustive-deps` warn), categoria `correctness`, `no-unused-vars` disattivato come nel vecchio ESLint. Criticità nota: oxlint **non** ha `react-refresh/only-export-components`, quindi quel controllo (solo warn, già disattivato sulle route) è stato lasciato cadere; è DX per Fast Refresh, non correttezza. Rimossi `eslint`, `@eslint/js`, `typescript-eslint`, `eslint-plugin-react-hooks`, `eslint-plugin-react-refresh`, `globals`. Il workflow GitHub linta in modo incrementale i file sorgente cambiati con `npx oxlint`.
