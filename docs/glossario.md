...
| **Bollo / Marca da bollo** | Importo fisso di 2,00 € applicabile alle fatture senza IVA sopra soglia quando la preferenza di fatturazione è attiva.                                              | In Pratix è disattivo di default e si abilita da Impostazioni → Fatturazione.                                                                               |
...
| **Server route**             | Endpoint HTTP definito in `src/routes/api.*.ts` e pubblicato tramite Vercel insieme all'app.                                        |
| **Edge function**            | Funzione serverless Supabase. Non è una superficie runtime corrente di Pratix: eventuali usi futuri richiedono decisione esplicita. |
