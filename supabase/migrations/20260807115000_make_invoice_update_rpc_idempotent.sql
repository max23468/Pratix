ALTER TABLE public.invoice_lines
  ADD COLUMN hearing_dates date[] NOT NULL DEFAULT '{}'::date[];

UPDATE public.invoice_lines il
SET hearing_dates = coalesce((
  SELECT array_agg(cah.hearing_date ORDER BY cah.position)
  FROM public.case_activity_hearings cah
  WHERE cah.activity_id = il.case_activity_id
    AND cah.user_id = il.user_id
), '{}'::date[])
WHERE il.case_activity_id IS NOT NULL;
