...
BEGIN
  ...
  ELSE
    ...
    VALUES (
      ...
      CASE
        WHEN v_normalized #>> '{client,kind}' = 'individual'
        THEN nullif(v_normalized #>> '{client,firstName}', '')
        ELSE NULL
      END,
      CASE
        WHEN v_normalized #>> '{client,kind}' = 'individual'
        THEN nullif(v_normalized #>> '{client,lastName}', '')
        ELSE NULL
      END,
      CASE
        WHEN v_normalized #>> '{client,kind}' = 'company'
        THEN nullif(v_normalized #>> '{client,businessName}', '')
        ELSE NULL
      END,
      'IT'
    )
  ...
  ELSE
    ...
    VALUES (
      ...
      CASE
        WHEN v_normalized #>> '{counterparty,kind}' = 'individual'
        THEN nullif(v_normalized #>> '{counterparty,firstName}', '')
        ELSE NULL
      END,
      CASE
        WHEN v_normalized #>> '{counterparty,kind}' = 'individual'
        THEN nullif(v_normalized #>> '{counterparty,lastName}', '')
        ELSE NULL
      END,
      CASE
        WHEN v_normalized #>> '{counterparty,kind}' <> 'individual'
        THEN nullif(v_normalized #>> '{counterparty,businessName}', '')
        ELSE NULL
      END,
  ...
  INSERT INTO public.principal_clients (
    user_id,
    principal_id,
    client_id,
    active_from
